// obs_rtmp_streaming.cpp - Multi-monitor OBS RTMP streaming with pause functionality for Windows
#include <obs.h>
...
private:
    ...
    // Pause control
    std::atomic<PauseState> pause_state{ PauseState::NONE };
    std::mutex pause_mutex;

    // Stored audio volumes for muting/unmuting
    float mic_volume = 1.0f;
    float desktop_audio_volume = 1.0f;

    // Stored visibility states for screen captures
    std::vector<bool> screen_capture_visibility;
    ...
    void pause_audio() {
        ...
        if (mic_capture) {
            mic_volume = obs_source_get_volume(mic_capture);
            obs_source_set_volume(mic_capture, 0.0f);
        }

        if (desktop_audio) {
            desktop_audio_volume = obs_source_get_volume(desktop_audio);
            obs_source_set_volume(desktop_audio, 0.0f);
        }

        std::cout << "Audio paused (muted)" << std::endl;
    }
    ...
    void resume_audio() {
        ...
        if (mic_capture) {
            obs_source_set_volume(mic_capture, mic_volume);
        }

        if (desktop_audio) {
            obs_source_set_volume(desktop_audio, desktop_audio_volume);
        }
    ...
    void pause_screen() {
        ...
        screen_capture_visibility.clear();

        // Hide all screen captures
        for (size_t i = 0; i < scene_items.size(); i++) {
            if (scene_items[i]) {
                screen_capture_visibility.push_back(obs_sceneitem_visible(scene_items[i]));
                obs_sceneitem_set_visible(scene_items[i], false);
            }
        }

        std::cout << "Screen capture paused" << std::endl;
    }
    ...
    void resume_screen() {
        ...
        for (size_t i = 0; i < scene_items.size() && i < screen_capture_visibility.size(); i++) {
            if (scene_items[i]) {
                obs_sceneitem_set_visible(scene_items[i], screen_capture_visibility[i]);
            }
        }

        screen_capture_visibility.clear();
        std::cout << "Screen capture resumed" << std::endl;
    }
    ...
    void control_loop() {
        ...
        std::cout << "Press 'A' to PAUSE/RESUME audio only" << std::endl;
        std::cout << "Press 'B' to PAUSE/RESUME both screen and audio" << std::endl;
        std::cout << "Press 'R' to RESUME all (if paused)" << std::endl;
        std::cout << "Press 'I' to show stream INFO/stats" << std::endl;
        ...
        while (!should_stop) {
            if (_kbhit()) {
                ...
                case 'A':
                    if (is_streaming) {
                        PauseState current = pause_state.load();
                        if (current == PauseState::NONE) {
                            pause_audio();
                            pause_state = PauseState::AUDIO_ONLY;
                            std::cout << "\nAudio paused. Press 'A' or 'R' to resume." << std::endl;
                        }
                        else if (current == PauseState::AUDIO_ONLY) {
                            resume_audio();
                            pause_state = PauseState::NONE;
                            std::cout << "\nAudio resumed." << std::endl;
                        }
                        else {
                            std::cout << "\nBoth audio and screen are paused. Use 'R' to resume all." << std::endl;
                        }
                    }
                    else {
                        std::cout << "\nNo stream is running!" << std::endl;
                ...
                case 'B':
                    if (is_streaming) {
                        ...
                        if (current == PauseState::NONE) {
                            pause_audio();
                            pause_screen();
                            pause_state = PauseState::BOTH;
                            std::cout << "\nBoth audio and screen paused. Press 'B' or 'R' to resume." << std::endl;
                        }
                        else if (current == PauseState::BOTH) {
                            resume_audio();
                            resume_screen();
                            pause_state = PauseState::NONE;
                            std::cout << "\nBoth audio and screen resumed." << std::endl;
                        }
                        else {
                            // If only audio is paused, pause screen too
                            pause_screen();
                            pause_state = PauseState::BOTH;
                            std::cout << "\nScreen also paused. Both audio and screen are now paused." << std::endl;
                        }
                    }
                    else {
                        std::cout << "\nNo stream is running!" << std::endl;
                ...
                case 'R':
                    if (is_streaming) {
                        PauseState current = pause_state.load();
                        if (current == PauseState::AUDIO_ONLY) {
                            resume_audio();
                            pause_state = PauseState::NONE;
                            std::cout << "\nAudio resumed." << std::endl;
                        }
                ...
                case 'I':
                    if (is_streaming) {
                        ...
                        // Show pause state
                        PauseState current = pause_state.load();
                        std::cout << "Pause state: ";
                        switch (current) {
                        case PauseState::NONE:
                            std::cout << "Not paused" << std::endl;
                            break;
                        case PauseState::AUDIO_ONLY:
                            std::cout << "Audio paused" << std::endl;
                            break;
                        case PauseState::BOTH:
                            std::cout << "Both audio and screen paused" << std::endl;
                            break;
                        }
                    }
                    else {
                ...
                case 'Q':
                    ...
                    if (is_streaming) {
                        // Resume everything before stopping
                        if (pause_state != PauseState::NONE) {
                            if (pause_state == PauseState::AUDIO_ONLY) {
                                resume_audio();
                            }
                            else if (pause_state == PauseState::BOTH) {
                                resume_audio();
                                resume_screen();
                            }
                            pause_state = PauseState::NONE;
                ...
                default:
                    std::cout << "\nUnknown command. Use S/T/A/B/R/I/Q" << std::endl;
                    break;
                }
            }
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cout << "OBS Multi-Monitor RTMP Streamer with Pause Support" << std::endl;
        std::cout << "=================================================" << std::endl;
        print_usage();
        return 1;
    }
